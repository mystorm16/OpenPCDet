...
def draw_lidars_box3d_on_birdview(lidars1, lidars2, colors, gt_box3d_center, input_map_h, input_map_w, xrange_min,
                                  xrange_max, yrange_min, yrange_max, vw=0.1, vh=0.1, bv_log_factor=1):
    image = lidar_to_bird_view_img(lidars1, input_map_h, input_map_w, xrange_min, xrange_max, yrange_min, yrange_max,
                                   vw, vh, bv_log_factor, color=colors[0])
    image = image.copy()
    image = lidar_to_bird_view_img(lidars2, input_map_h, input_map_w, xrange_min, xrange_max, yrange_min, yrange_max,
                                   vw, vh, bv_log_factor, color=colors[1], birdview=image)
    img = image.copy()
...
def lidar_to_bird_view_img(lidar, input_map_h=794, input_map_w=692, xrange_min=0, xrange_max=69.12, yrange_min=-39.68,
                           yrange_max=39.68, vw=0.1, vh=0.1, factor=1, color=(255, 255, 255), birdview=None):
    if birdview is None:
        birdview = np.zeros(
