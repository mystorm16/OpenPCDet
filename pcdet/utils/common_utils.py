...
# sy_add 将box内的点绕着box的x轴对称
def symmetry_box_x(points, angle_global, angle_box):
    points, is_numpy = check_numpy_to_torch(points)
    angle_global, _ = check_numpy_to_torch(angle_global)
    angle_box, _ = check_numpy_to_torch(angle_box)

    # global的旋转矩阵
    cosa = torch.cos(angle_global)
    sina = torch.sin(angle_global)
    zeros = angle_global.new_zeros(points.shape[0])
    ones = angle_global.new_ones(points.shape[0])
    rot_matrix_global = torch.stack((
        ones, zeros, zeros,
        zeros, cosa, sina,
        zeros, -sina, cosa
    ...
    # box的旋转矩阵
    cosa = torch.cos(angle_box)
    sina = torch.sin(angle_box)
    zeros = angle_box.new_zeros(points.shape[0])
    ones = angle_box.new_ones(points.shape[0])
    rot_matrix_box = torch.stack((
        cosa, -sina, zeros,
        sina, cosa, zeros,
        zeros, zeros, ones
    ), dim=1).view(-1, 3, 3).float()
    rot_matrix_box_inv = torch.linalg.inv(rot_matrix_box)
    points_rot = torch.matmul(points[:, :, 0:3], rot_matrix_box)  # 转到归一化系
    points_rot = torch.matmul(points_rot, rot_matrix_global)  # 绕x轴翻转180
    points_rot[0][:, 2] = -points_rot[0][:, 2]  # 翻转之后y轴需要取相反数
    points_rot = torch.matmul(points_rot, rot_matrix_box_inv)  # 从归一化系转回来
    points_rot = torch.cat((points_rot, points[:, :, 3:]), dim=-1)
    return points_rot.numpy() if is_numpy else points_rot


# sy_add 将box内的点绕着box的x轴对称 归一化坐标系
def symmetry_box_x_csc(points, angle_box, angle_global):
    points, is_numpy = check_numpy_to_torch(points)
    angle_global, _ = check_numpy_to_torch(angle_global)
    angle_box, _ = check_numpy_to_torch(angle_box)
    ...
    # box的旋转矩阵 即在绕着x轴旋转180
    cos = torch.cos(angle_box)
    sin = torch.sin(angle_box)
    zero = angle_box.new_zeros(points.shape[0])
    one = angle_box.new_ones(points.shape[0])
    rot_matrix_box = torch.stack((
        one, zero, zero,
        zero, cos, sin,
        zero, -sin, cos
    ...
    points_csc = torch.matmul(points[:, :, 0:3], rot_matrix_global)  # 转到归一化系
    V.draw_scenes(
        points=points_csc[0],
        draw_origin=True
    )
    points_rot = torch.matmul(points_csc, rot_matrix_box)  # 绕x轴翻转180
    points_rot[0][:, 2] = -points_rot[0][:, 2]  # 翻转之后y轴需要取相反数

    points_csc = torch.cat((points_csc, points[:, :, 3:]), dim=-1)  # 把反射强度拼接回去
    points_rot = torch.cat((points_rot, points[:, :, 3:]), dim=-1)  # 把反射强度拼接回去
    V.draw_scenes(
        points=torch.cat((points_csc[0], points_rot[0]), dim=0),
        draw_origin=True
    )
    return points_rot, points_csc
