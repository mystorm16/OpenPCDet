...
class VoxelQuery(Function):
    ...
    def forward(ctx, max_range: int, radius: float, nsample: int, xyz: torch.Tensor, \
        ...
        idx = torch.cuda.IntTensor(M, nsample).zero_()  # 每个gird point邻域的16个位置 用index记录
...
class VoxelQueryAndGrouping(nn.Module):
    ...
    def forward(self, new_coords: torch.Tensor, xyz: torch.Tensor, xyz_batch_cnt: torch.Tensor,
        ...
        """
        Args:
            new_coords: (M1 + M2 ..., 3) centers voxel indices of the ball query、、球查询的中心体素index
            xyz: (N1 + N2 ..., 3) xyz coordinates of the features、、原始点云场景
            xyz_batch_cnt: (batch_size), [N1, N2, ...]  、、batch index
            new_xyz: (M1 + M2 ..., 3) centers of the ball query、、球查询中心体素坐标
            new_xyz_batch_cnt: (batch_size), [M1, M2, ...] 、、batch index
            features: (N1 + N2 ..., C) tensor of features to group、、整个场景voxel的特征图
            voxel2point_indices: (B, Z, Y, X) tensor of points indices of voxels、、voxel对应点云的index
        ...
        # real_grouped_xyz = grouped_xyz[~empty_ball_mask]
        # vis_grouped_xyz = torch.transpose(real_grouped_xyz, 1, 2).reshape(-1, 3)
        # vis_grouped_xyz = torch.transpose(grouped_xyz, 1, 2).reshape(-1, 3)
        # V.draw_scenes(vis_grouped_xyz)
        # V.draw_scenes(torch.vstack((vis_grouped_xyz, new_xyz)))
        # V.draw_scenes(torch.vstack((vis_grouped_xyz, new_xyz, xyz)))
        # V.draw_scenes(xyz)
        # V.draw_scenes(new_xyz)
