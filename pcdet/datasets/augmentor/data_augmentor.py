...
class DataAugmentor(object):
        ...
        for cur_cfg in aug_config_list:
            if not isinstance(augmentor_configs, list):
                if cur_cfg.NAME in augmentor_configs.DISABLE_AUG_LIST:
                    continue
            if (cur_cfg.NAME in ["waymo_gt_sampling", "gt_sampling", "add_best_match", "sup_gt_sampling"]) and len(
                    self.db_infos.keys()) == 0:
    ...
    def random_world_rotation(self, data_dict=None, config=None):
        ...
        special_points_lst = [data_dict[pt_key] for pt_key in SPECIAL_NAMES if pt_key in data_dict]
        special_name_lst = [pt_key for pt_key in SPECIAL_NAMES if pt_key in data_dict]
        gt_boxes, points, noise_rotation, special_points_lst = augmentor_utils.global_rotation(data_dict['gt_boxes'],
                                                                                               pre_rot_points,
                                                                                               rot_range=rot_range,
                                                                                               special_points_lst=special_points_lst)
        ...
        # draw_scenes(data_dict['pre_rot_points'])
        # draw_scenes(data_dict['points'])
        return data_dict
    ...
    def forward(self, data_dict, validation=False):
        ...
        # draw_scenes(data_dict['bm_points'])
