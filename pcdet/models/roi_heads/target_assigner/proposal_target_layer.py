...
class ProposalTargetLayer(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        Returns:
            batch_dict:
                ...
                rcnn_cls_labels: (B, M)
        """
        # 为第二阶段rcnn网络 选取第一阶段的roi结果和对应的gt box
        ...
        # 对iou大于0.6的roi进行分类，忽略iou属于0.45到0.6之间roi的loss计算
    ...
    def sample_rois_for_rcnn(self, batch_dict):
        ...
        for index in range(batch_size):
            ...
            if self.roi_sampler_cfg.get('SAMPLE_ROI_BY_EACH_CLASS', False):
                # 使用self.get_max_iou_with_same_class()函数计算512个proposal与对应类别GT的3D IOU大小。
            ...
            # self.subsample_rois（）函数完成128个proposal采样和proposal的正负样本分配。
    ...
    def subsample_rois(self, max_overlaps):
        ...
        fg_num_rois = fg_inds.numel()  # 512个第一阶段结果里前景roi的数量
        bg_num_rois = hard_bg_inds.numel() + easy_bg_inds.numel()  # 512个第一阶段结果里背景roi的数量

        # 如果该帧中，前景的roi大于0,并且背景的roi也大于0
        if fg_num_rois > 0 and bg_num_rois > 0:
            # print("*********", fg_num_rois, "*************", "\n")
            # 能进这里面的fg_num_rois数量都很少，就几个
