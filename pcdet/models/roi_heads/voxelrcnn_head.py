...
class VoxelRCNNHead(RoIHeadTemplate):
    ...
    def forward(self, batch_dict):
        ...
        # 从第一阶段结果里通过NMS选取   100个结果test\512个结果trian  # 这里NMS用的voxelhead的第一个NMS配置,其实用不上
        targets_dict = self.proposal_layer(
            batch_dict, nms_config=self.model_cfg.NMS_CONFIG['TRAIN' if self.training else 'TEST']
        )

        if self.training:
            # assign_targets:根据第一阶段输出结果整理出正负样本为1：1送给第二阶段网络。
            # 具体的方法是判断第一阶段ROI和GT的iou，大于一定阈值是前景ROI也就是正样本，小于一定阈值是负样本，最后拼接正负样本
        ...
        # Box Refinement
        pooled_features = pooled_features.view(pooled_features.size(0), -1)  # （ROI数量，6*6*6*C）
        shared_features = self.shared_fc_layer(pooled_features)  # （ROI数量，256）
        rcnn_cls = self.cls_pred_layer(self.cls_fc_layers(shared_features))  # （ROI数量，1）
        rcnn_reg = self.reg_pred_layer(self.reg_fc_layers(shared_features))  # （ROI数量，7）
