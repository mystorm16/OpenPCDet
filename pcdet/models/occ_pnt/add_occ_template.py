...
class AddOccTemplate(nn.Module):
    ...
    # 在200 176 9的预测结果tensor中找大于0.3的voxel，再取预测值最大的2048个结果用于训练
    def filter_occ_points(self, batch_size, occ_probs, batch_dict):
        ...
        for i in range(batch_size):
            occ_mask = occ_probs[i] > self.occ_thresh  # 9 157 209逐体素和阈值比较
            occ_coords = torch.nonzero(occ_mask)  # 大于阈值的体素
            numpass = torch.sum(occ_mask)  # 大于阈值的体素数量
            if numpass > 0 and batch_dict["use_occ_prob"][i]:
                top_prob = occ_probs[i][occ_mask]  # 每个大于阈值的体素的occupancy概率
                if self.reg:
                    top_pnt = batch_dict["pred_sem_residuals"][i][:, occ_mask].permute(1, 0)  # 大于阈值的体素对应的pred_sem_residuals
                ...
                if top_prob.shape[0] > max_add_occpnts_num:  # 在阈值大于0.3的voxel中取最大的2048个
                ...
                top_occ_coords = torch.cat((torch.unsqueeze(torch.ones_like(occ_coords[..., 0], device=occ_coords.device,
                                                                            dtype=occ_coords.dtype), -1) * i, occ_coords), axis=-1)
    ...
    def occ_coords2absxyz(self, occ_coords, rot_z=None):
