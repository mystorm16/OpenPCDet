...
class OccHead3D(OccHeadTemplate):
    def __init__(self, model_cfg, data_cfg, input_channels, num_class, grid_size):
        self.is_softmax = model_cfg.OCC_DENSE_HEAD.LOSS_CONFIG.get("CLS_LOSS_TYPE",
                                                                   None) is not None and model_cfg.OCC_DENSE_HEAD.LOSS_CONFIG.CLS_LOSS_TYPE == "softmax"
        super().__init__(
        ...
        self.conv_cls = spconv.SparseSequential(
            spconv.SubMConv3d(input_channels, (self.stride ** 3) * cls_channel, 3, padding=1, bias=True,
                              indice_key='cls_ind'),
        )
        self.logit2prob = torch.nn.Softmax(dim=1) if self.is_softmax else torch.nn.Sigmoid()

        if self.reg:
            self.conv_res = spconv.SparseSequential(
                spconv.SubMConv3d(input_channels, (self.stride ** 3) * self.num_class * self.res_num_dim, 3, padding=1,
                                  bias=False, indice_key='res_ind'))
    ...
    def forward(self, data_dict):
        spatial_features = data_dict['encoded_spconv_tensor']  # 每个不为空位置的32维特征
        # print("spatial_features.shape", spatial_features.shape)
        logit_preds = self.conv_cls(spatial_features).dense()  # sub3D 【9 157 209 32】->【1 2 9 157 209 32】
        prob_preds = self.logit2prob(logit_preds)[:, -1:, ...]  # can fit both sigmoid and softmax 【1 1 9 157 209】
