...
from tools.visual_utils.open3d_vis_utils import draw_spherical_voxels_4, draw_spherical_voxels_index, \
    draw_scenes_voxel_b, draw_scenes_voxel_a
...
class OccHeadTemplate(nn.Module):
    def __init__(self, model_cfg, data_cfg, num_class, grid_size):
        ...
        self.noloss = self.model_cfg.OCC_DENSE_HEAD.get("NOLOSS",
                                                        None) is not None and self.model_cfg.OCC_DENSE_HEAD.NOLOSS
        self.num_class = num_class
        self.forward_ret_dict = {}
        self.nx, self.ny, self.nz = grid_size
        self.occ_fore_res_weight = self.model_cfg.OCC_DENSE_HEAD.LOSS_CONFIG.LOSS_WEIGHTS.get("occ_fore_res_weight",
                                                                                              0.1)
        self.occ_fore_cls_weight = self.model_cfg.OCC_DENSE_HEAD.LOSS_CONFIG.LOSS_WEIGHTS.get("occ_fore_cls_weight",
                                                                                              1.0)
    ...
    def build_losses(self, losses_cfg):
        ...
        if self.reg:
            reg_loss_name = 'WeightedSmoothL1Loss' if losses_cfg.get('REG_LOSS_TYPE',
                                                                     None) is None else losses_cfg.REG_LOSS_TYPE
            self.add_module('reg_loss_func',
                            getattr(loss_utils, reg_loss_name)(beta=losses_cfg.LOSS_WEIGHTS['res_beta'],
                                                               code_weights=[1.0 for i in range(self.res_num_dim)]))
    ...
    def get_res_layer_loss(self, batch_dict):
        ...
        if self.num_class > 1:
            ...
            preds_sem_res = preds_sem_res.view(-1, self.num_class, self.res_num_dim)[self.first_perm, label_ind,
                            :].view(bs, self.ny, self.nx, self.nz * self.sz * self.sy * self.sx, self.res_num_dim)
        reg_loss = self.mean_masked_loss(preds_sem_res, res_mtrx, self.reg_loss_func, general_reg_loss_mask_float,
                                         mask=general_reg_loss_mask) * self.occ_fore_res_weight
    ...
    def get_cls_layer_loss(self, batch_dict):
        general_cls_loss_mask_float = batch_dict["general_cls_loss_mask_float"].unsqueeze(1)  # 信息缺失区域(含权重)
        # draw_scenes_voxel_b(general_cls_loss_mask_float[0])
        pred_occ_logit = batch_dict['pred_occ_logit']
        pos_mask = batch_dict["pos_mask"].to(pred_occ_logit.dtype)  # pos_mask是最终补全的点云
        # draw_scenes_voxel_b(pos_mask)
        # complimentary_mask是在整个形状中剔除pos_mask  目的是构建整个形状的真值标签，pos_mask位置是0其余位置是1
        complimentary_mask = torch.ones_like(pos_mask, device=pos_mask.device, dtype=pos_mask.dtype) - pos_mask
        # draw_scenes_voxel_b(complimentary_mask)
        ...
        cls_loss = self.mean_masked_loss(pred_occ_logit, one_hot_targets, self.cls_loss_func,
                                         general_cls_loss_mask_float,
                                         mask=batch_dict['general_cls_loss_mask']) * self.occ_fore_cls_weight
        tb_dict = {
    ...
    def mean_masked_loss(self, pred, target, loss_func, loss_weight_float, mask=None):
        ...
        # draw_spherical_voxels_4(mask[:, 1:])
        inds = mask.nonzero() if mask is not None else (loss_weight_float > 1e-4).nonzero()  # 信息缺失区域
        # draw_scenes_voxel_a(inds)
        loss_weight_float = loss_weight_float[inds[:, 0], :, inds[:, 1], inds[:, 2],
                            inds[:, 3]]  # 从loss_weight_float中抠出信息缺失区域的权重
        target = target[inds[:, 0], :, inds[:, 1], inds[:, 2], inds[:, 3]]  # 从target中抠出信息缺失区域的标签【0 1】或【1 0】
        pred = pred[inds[:, 0], :, inds[:, 1], inds[:, 2], inds[:, 3]]  # 从pred中抠出信息缺失区域的预测结果
